    ...
    let research = function_node("research", |_state: &MessagesState| async move {
        ...
        update.insert(
            ...
            serde_json::to_value(vec![Message::new_ai_message(
                "replace this node with your real workflow",
            )])?,
        );
        Ok(update)
    });
...
async fn main() -> Result<()> {
    tracing_subscriber::fmt()
        .with_env_filter(EnvFilter::try_from_default_env().unwrap_or_else(|_| {
            EnvFilter::new("info,oris_runtime=info")
        }))
        .init();

    let db_path = std::env::var("ORIS_SQLITE_DB").unwrap_or_else(|_| "oris_template.db".into());
