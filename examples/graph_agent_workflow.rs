...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    while let Some(event) = stream.next().await {
        match event {
            oris::graph::StreamEvent::NodeStart { node, .. } => {
                println!("  → Starting node: {}", node);
            }
            oris::graph::StreamEvent::NodeEnd { node, .. } => {
                println!("  ← Completed node: {}", node);
            }
            oris::graph::StreamEvent::GraphEnd { .. } => {
                println!("  ✓ Graph completed");
            }
            oris::graph::StreamEvent::Error { error } => {
                eprintln!("  ✗ Error: {:?}", error);
            }
            oris::graph::StreamEvent::MessageChunk { .. }
            | oris::graph::StreamEvent::CustomData { .. } => {}
        }
    }
